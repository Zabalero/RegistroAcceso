...
if (es_usuario ( $_SESSION ['usuario'], $_SESSION ['password'] )) {
	if ($rolUsuario == 'Avanzado' || $rolUsuario == 'Insercion' || $rolUsuario == 'Lectura') {
		...
		if ($rolUsuario == 'Avanzado') {
			...
			echo '<li><a href="./adProvincias.php">Aï¿½adir provincias</a></li>';
			echo '<li><a href="./adPoblaciones.php">Aï¿½adir poblaciones</a></li>';
			echo '</ul>';
			echo '</div>';
			echo '<li class="line">|</li>';
		...
		if (isset ( $modificar )) {
			...
			$tsql = trim ( $tsql, ',' ); // quita ï¿½ltima coma
			$tsql = $tsql . " WHERE idAcceso='" . $id . "'";
			$stmt = sqlsrv_query ( $conn, $tsql ) or die ( "Fallo en la actualizaciï¿½n" );
		...
		<div class="izq">
			...
			<?PHP
				if ($row ["Autorizado"] == 0) {
					...
					print ("<option value=1>Sï¿½</option>") ;
				} else {
					print ("<option value=1 selected>SÃ­</option>") ;
