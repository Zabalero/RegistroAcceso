	...
	if ($dni!='' && $poblacion!='' && $camara!=''){
		if (strlen($camara)==8 && (eregi(cr,$camara) || eregi(ad,$camara))){	
			...
			// Conectar con el servidor de base de datos
				...
				//if (isset($tecnicoSelecc)){ //entrada datos con idTécnico, no manual
				...
				*/
					...
					if (isset($tecnicoSelecc))//si se selecciona un técnico de la lista (entrada no manual)
			...
			sqlsrv_close( $conn);
			 ...
			 /* } //fin actuación válida
		   
			else{
			echo "<script>alert('No se actualizan datos, inserte un técnico válido')</script>";
			}
...
//después de seleccionar el técnico de la tabla
					...
					<?php		
								...
								if (isset($_POST['Submit2']) || isset($_POST['Submit1'])){
									...
									if ($rows === true){
										...
										echo "<script>alert('No existe el técnico en la base de datos. Debe introducir sus datos manualmente')</script>";
									}
				...
				<div id="bloque2">	<!--bloque2-->				
					...
					*/
					...
					<div class=izq>
						...
						<input type="radio" name="tipo" value="2" checked>Incidencia Remedy
					</div>
					<div class="der">	
